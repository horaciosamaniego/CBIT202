...
* Nos evita guardar muchos objetos intermedios en memoria. Por ejemplo:
...
```{r, echo=FALSE}
Summary.Byspecies <- summarize(
  group_by(
    mutate(iris, 
           Petal.Sepal.Ratio = Petal.Length/Sepal.Length), Species), 
...
## Pipeline, otro ejemplo {.small .build}
...
library(knitr)
MEAN <- iris |> group_by(Species) |> summarize_all(.funs = list(Mean = mean, SD =sd))
kable(MEAN) |> kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T,font_size = 6)
```
# Mas dudas?
...
```{r, echo = FALSE}
DF <- iris |> filter(Petal.Length >= 4 & Sepal.Length >= 5) |> group_by(Species) |> summarise(N = n())
kable(DF) |> kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T)
```
...
```{r, echo = FALSE}
DF |>
  dplyr::select(c("Petal.Length_Mean","Sepal.Length_Mean")) |>
  kable() |> 
...
fname <- "https://raw.githubusercontent.com/jorgeperezrojas/covid19-data/master/csv/confirmados_comunas.csv"
Casos_Activos <- read.csv(fname)
...
* ¿Cuáles son las 10 comunas que han tenido la mayor mediana de prevalencia? Para cada una de estas 10 comunas, genera una tabla con la mediana, prevalencia máxima y fecha en que se alcanzó la prevalencia máxima.
